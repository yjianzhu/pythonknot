...
------------------------------------------------------------------------------
*/
int list[BIGWEAVE];              /* description of first new weave */
int list2[BIGWEAVE];           /* description of second, if needed */
int old_going_in[BIGWEAVE];/* Was *i* an input? *old_going_in[i]*. */
int going_in[BIGWEAVE];    /* Will *i* be an input? *going_in[i]*. */
int map[BIGWEAVE];   /* i of old weave becomes map[i] of new weave */
int first;                    /* first boundary crossing to remove */
int second;                  /* second boundary crossing to remove */
int right;          /* Is the crossing being added be righthanded? */
int oldcross;     /* number of boundary crossings in the old weave */
int newcross;    /* number of boundary crossings in each new weave */
int oldin;                    /* number of inputs to the old weave */
int newin;                    /* number of inputs to the new weave */
...
------------------------------------------------------------------------------
*/
void  b_no_pairs(int  *list,  /* list of original inputs/outputs, modified by this routine */
                 int  *list2,                    /* inputs/outputs of the second new weave */
                 int  *one,                                 /* will one new weave suffice? */
                 int  *two)                                /* will two new weaves suffice? */
{
  int  i,
        a,
        b,
...
void b_one_pair(int  *list, int  *list2, int  *one,  int  *two )  
   /* list of original inputs/outputs, modified by this routine */
                     /* inputs/outputs of the second new weave */
                                /* will one new weave suffice? */
                              /* will two new weaves suffice? */
{
  int   a;
  int   i;
  int   crossed;
